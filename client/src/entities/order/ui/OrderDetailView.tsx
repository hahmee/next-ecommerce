...
export function OrderDetailView({ orders, isLoading }: Props) {
        ...
        <div className="mb-15">
          ...
          <ul className="border border-gray-200 rounded p-3 text-xs">
            ...
            <li className="flex justify-between py-1.5">
              ...
              <span>{first.createdAt ? dayjs(first.createdAt).format('YYYY.MM.DD') : '-'}</span>
            </li>
            <li className="flex justify-between py-1.5">
              <span className="text-gray-500">주문상태</span>
              <span>{getOrderStatusText(first.status)}</span>
            </li>
            <li className="flex justify-between py-1.5">
              <span className="text-gray-500">총금액</span>
              <span>{first.totalAmount.toLocaleString()} 원</span>
            </li>
          </ul>
        </div>
                ...
                <div className="flex gap-3 justify-between">
                  <div className="flex gap-x-3">
                    <Link href={`/product/${item.productInfo.pno}`}>
                      <Image
                        src={item.productInfo.thumbnailUrl || '/images/mall/no_image.png'}
                        alt="image"
                        width={500}
                        height={500}
                  ...
                  <Link href={`/review-write?oid=${item.id}&orderId=${item.orderId}`}>
                    <button
                      type="button"
                      className="w-28 text-xs rounded ring-1 ring-ecom text-ecom py-2 px-4 hover:bg-ecom hover:text-white disabled:cursor-not-allowed disabled:bg-pink-200 disabled:ring-0 disabled:text-white"
                    >
                      리뷰쓰기
                    </button>
                  </Link>
        ...
        <div>
          ...
          <ul className="border border-gray-200 rounded p-3 text-xs">
            <li className="flex justify-between py-1.5">
              ...
              <span>{first.deliveryInfo.receiver}</span>
            </li>
            <li className="flex justify-between py-1.5">
              <span className="text-gray-500">연락처</span>
              <span>{first.deliveryInfo.phone}</span>
            </li>
            <li className="flex justify-between py-1.5">
              <span className="text-gray-500">주소</span>
              <span>{first.deliveryInfo.address}</span>
            </li>
            <li className="flex justify-between py-1.5">
              <span className="text-gray-500">배송메시지</span>
              <span>{first.deliveryInfo.message}</span>
            </li>
          </ul>
        </div>
      </div>
