...
const ProductOrders = () => {
    ...
    return <div className="relative">
        <div onClick={() => setSortOpen((open) => !open)}
        ...
        {
            sortOpen && <div className={`w-full absolute p-4 rounded-md shadow-[0_3px_10px_rgb(0,0,0,0.2)] bg-white top-12 left-0 z-20 animate-fadeInUp`}>
                {
                    orders.map((order) =>
                        <div key={order.id} className="flex items-center my-2">
                            <input id={order.id} defaultChecked={order === orderValue} type="radio" value={order.id}
                                   name="order-radio"
                            ...
                            <label htmlFor={order.id}
                                   className="ms-2 text-base font-medium text-gray-900 dark:text-gray-300">{order.name}</label>
                        </div>
                    )
                }
            </div>
