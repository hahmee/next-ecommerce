...
const NavIcons = () => {
  ...
  const {
    ...
    error,
    isError,
  } = useQuery<Array<CartItemList>, Object, Array<CartItemList>>({
    queryKey: ['carts'],
    queryFn: () => getCart(),
  ...
  return (
    <div className="flex items-center gap-4 xl:gap-6 relative">
      {/* 모달 바깥 클릭 */}
      <div
        className={`z-10 fixed w-full overflow-hidden h-screen top-0 left-0 ${!accountOpen && 'hidden'}`}
      ...
      {user?.roleNames?.some((role) =>
        [MemberRole.ADMIN, MemberRole.MANAGER, MemberRole.DEMO].includes(role),
      ) && (
