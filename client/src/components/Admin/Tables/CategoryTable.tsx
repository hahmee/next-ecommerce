...
const CategoryTable = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // 기존 데이터 가져오기
      const previousData: PageResponse<CategoryTree> | undefined = queryClient.getQueryData([
        'adminCategories',
        { page, size, search },
      ]);
    ...
    <div className="bg-white dark:bg-gray-800 shadow-md rounded-sm">
      ...
      <div className="w-auto overflow-x-auto overflow-y-hidden relative">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400 relative">
          ...
          <tbody>
            {categoryData?.dtoList && categoryData?.dtoList.length > 0 ? (
              renderCategoryRows(categoryData.dtoList || [])
            ) : (
              <tr>
                <td
                  scope="row"
