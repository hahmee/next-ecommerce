...
export function ProductTableView(props: {
  ...
  const {
    ...
    showDialog, clickModal, requestDelete, confirmDelete,
  } = props;
    ...
    <div className="bg-white dark:bg-gray-800 shadow-md rounded-sm ">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
        ...
        <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
          <TableAddButton content="Add Product" location="/admin/products/add-product" ariaLabel="Add Product" />
      ...
      <div className="w-auto overflow-x-auto overflow-y-hidden">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          {rows.length > 0 ? (
            rows.map((product) => (
              <tr className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700" key={product.pno}>
                  ...
                  <TableActions>
                    ...
                    >
                      <ul className="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="table-dropdown-button">
                        <li>
                          <Link href={`/product/${product.pno}`} className="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                            상품보기
                          </Link>
                        </li>
                        <li>
                          <Link href={`/admin/products/${product.pno}`} className="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                            수정하기
                          </Link>
                        </li>
                      </ul>
            ...
            <tr>
              <td colSpan={11} className="text-center px-4 py-3 text-gray-500 whitespace-nowrap dark:text-white">
                No results
              </td>
            </tr>
          )}
      ...
      <div className="px-4 py-6 md:px-6 xl:px-7.5">
        <PageComponent pagingData={paging} size={size} search={props.search} changePage={changePage} />
      </div>
    </div>
  );
}
