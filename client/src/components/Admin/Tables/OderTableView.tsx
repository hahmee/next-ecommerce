...
export function OrderTableView(props: {
  ...
  const {
    list, paging,
    page, size, search, date,
    expandedRows, isFetching,
    handleSearch, changeSize, changePage, toggleRow, dateChange,
  } = props;
    ...
    <div className="bg-white dark:bg-gray-800 shadow-md rounded-sm">
      ...
      <div className="w-auto overflow-x-auto overflow-y-hidden relative">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400 relative">
            ...
            <Fragment key={payment.id}>
              ...
              {expandedRows.includes(payment.id) && !!payment.orders?.length && (
                <tr className="border-b dark:border-gray-700">
                  <th scope="row" colSpan={7} className="pl-4 py-1 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                      ...
                      {payment.orders.map((o, idx) => (
                        <div key={idx} className="w-auto grid grid-cols-3 gap-3 my-2">
                          ...
                          <div>
                            ...
                            <div className="font-light">가격: {o.productInfo.price.toLocaleString()}</div>
                            <div className="font-light">색상: {o.productInfo.color.text}</div>
                            <div className="font-light">사이즈: {o.productInfo.size}</div>
                          </div>
                          <div className="font-light text-gray-500">X {o.productInfo.qty.toLocaleString()}</div>
                        </div>
                      ))}
                    </div>
                  </th>
          ...
          {list.length === 0 && (
            <tr>
              <td colSpan={7} className="text-center px-4 py-6">No results</td>
            </tr>
          )}
          </tbody>
        </table>
