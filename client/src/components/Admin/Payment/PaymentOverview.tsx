...
const PaymentOverview = () => {
  const getDateRange = (days: number) => {
    const end = dayjs(); // 오늘
    const start = end.subtract(days - 1, 'day'); // N일 전부터 오늘까지
    return {
  ...
  const [date, setDate] = useState(getDateRange(30));
  ...
  const paymentMenuOption: Array<Option<string>> = [
    ...
    { id: 'today', content: '오늘', ...getDateRange(1) },
    ...
    <div className="grid grid-cols-2 grid-rows-[auto,1fr] divide-y rounded-sm border shadow-default dark:border-strokedark dark:bg-boxdark bg-white">
      ...
      <div className="border-r flex flex-col gap-2 p-7.5">
        ...
        <div className="font-semibold text-base">
          {paymentSummary?.totalAmount.toLocaleString()} 원
        </div>
      </div>
