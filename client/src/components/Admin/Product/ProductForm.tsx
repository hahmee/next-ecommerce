...
const ProductForm = ({ type, id }: Props) => {
  ...
  const { isLoading, data: originalData } = useQuery<
    Product,
    Object,
    Product,
    [_1: string, _2: string]
    ...
    async onSuccess(response, variable) {
      ...
      if (queryClient.getQueryData(['adminProducts', { page: 1, size: 10, search: '' }])) {
        queryClient.setQueryData(
          ['adminProducts', { page: 1, size: 10, search: '' }],
          (prevData: { dtoList: Product[] }) => {
            if (type === Mode.ADD) {
              prevData.dtoList.unshift(newProduct);
            } else {
              prevData.dtoList = prevData.dtoList.map((product) =>
                product.pno === newProduct.pno ? newProduct : product,
              );
            }
      ...
      if (queryClient.getQueryData(['productSingle', newProduct.pno.toString()])) {
        queryClient.setQueryData(
          ['productSingle', newProduct.pno.toString()],
          (prevData: Product) => {
            return newProduct;
          },
        );
          ...
          <div className="flex flex-col gap-9">
            <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
              <div className="flex justify-between border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                ...
                <Link href="/admin/category" className="underline text-sm">
                  카테고리 추가/변경
                </Link>
              </div>
