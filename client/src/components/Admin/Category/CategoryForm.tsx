...
const CategoryForm = ({ type, id }: Props) => {
  ...
  const { data: originalData } = useQuery<Category, Object, Category, [_1: string, _2: string]>({
  ...
  const { data: categoryPaths } = useQuery<
    Category[],
    Object,
    Category[],
    [_1: string, _2: string]
  ...
  const mutation = useMutation({
    mutationFn: async (form: FormValues) => {
      ...
      return fetcher(`/api/category/${id}`, {
        method: 'PUT',
        credentials: 'include',
        body: formData,
      });
    ...
    onSuccess: async (newCategory: Category) => {
      ...
      queryClient.setQueryData(
        ['categoryPaths', newCategory.cno.toString()],
        (prev: Category[] = []) => prev.map((c) => (c.cno === newCategory.cno ? newCategory : c)),
      );
